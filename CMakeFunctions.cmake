...
function(getRecursiveSourceFiles targetName dir group publicHeaders)
    ...
    foreach(child ${children})
        if (IS_DIRECTORY ${dir}/${child})
            ...
            #message("Targetname: ${targetName} - Source lib: ${libBinPath}")
            if ("${libBinPath}" STREQUAL "/home/prive/projecten/shareD/nappgui/lib/gcc11_gtk3_x64/libshared-api.a")
                #message("We can skip this")
            else()
                #message("No we cant")
...
function(processTarget targetName targetType dependList)
    ...
    # Get Files
    if (targetName STREQUAL "shared-api")
    else()
    ...
    # TARGET Build
    if (targetName STREQUAL "shared-api")
        message(STATUS "- [OK] ${targetName}: .:: IMPORTED ::.")
        add_library(${targetName} SHARED IMPORTED)
        set_target_properties(
            ${targetName} PROPERTIES
            IMPORTED_LOCATION_DEBUG "/home/prive/projecten/shareD/build"
        )
    ...
    elseif (targetType STREQUAL STATIC_LIB)
    ...
    foreach(config ${CMAKE_CONFIGURATION_TYPES})
    if (TARGET "shared-api")
        #install(FILES ${targetName} CONFIGURATIONS "${config}" DESTINATION "lib/${toolset}/${config}" PERMISSIONS ${RTPERM} OPTIONAL)
    else()
...
function(existsPrecompiledDynamicLib libName _ret)
    ...
    if ("shared-api" STREQUAL ${libName})
        set(${_ret} "TRUE" PARENT_SCOPE)
        return()
    endif()
...
function(targetLinkWithLibraries targetName firstLevelDepends)
    ...
    if (${targetName} STREQUAL "shared-api")
        target_link_libraries(${targetName} INTERFACE "m")
    elseif (${CMAKE_SYSTEM_NAME} STREQUAL "Linux")
...
function(processInstaller targetName)
    if (CMAKE_PACKAGE_PATH)
        ...
        message("${targetName} kefjbsejkbsefjk")
...
function(generateSolution)
    ...
    foreach(sourcePath ${NAPPGUI_CACHE_SOURCE_DIRECTORIES})
        ...
        if ("/home/prive/projecten/shareD/build" STREQUAL "${CMAKE_CURRENT_SOURCE_DIR}/${sourcePath}")
            message("WHEN THIS COMES, EVERYTHING SHOULD BE FINE")
        elseif (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${sourcePath})
